...
typedef struct {
    deviceonly( __global ) snapshot_t* buffer;
    k64u bitmap;
} snapshot_manager_t;
...
k8u snapshot_save( rtinfo_t* rti, deviceonly( __global ) snapshot_t** snapshot ) {
    k8u no;
...
deviceonly(__global) snapshot_t* snapshot_kill( rtinfo_t* rti, k8u no ) {
...
deviceonly(__global) snapshot_t* snapshot_read( rtinfo_t* rti, k8u no ) {
    assert( no < SNAPSHOT_SIZE ); /* Subscript check. */
    assert( ~rti->snapshot_manager.bitmap & ( K64U_MSB >> no ) ); /* Existance check. */
    return &( rti->snapshot_manager.buffer[ no ] );
}

void snapshot_init( rtinfo_t* rti, deviceonly( __global ) snapshot_t* buffer ) {
...
void sim_init( rtinfo_t* rti, k32u seed, deviceonly( __global ) snapshot_t* ssbuf ) {
...
deviceonly(__kernel) void sim_iterate( deviceonly( __global ) float* dps_result, deviceonly( __global ) snapshot_t* snapshot_buffer ) {
